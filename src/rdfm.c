...
static void _apply_keybinds_from_file(const char *path)
{
    ...
    for (int g = 0; groups && groups[g]; g++) {
        gchar **keys = g_key_file_get_keys(kf, groups[g], NULL, NULL);
...
static void rdfm_load_keybinds(void)
{
    ...
    if (g_file_test(user_path, G_FILE_TEST_EXISTS)) {
        _apply_keybinds_from_file(user_path);
        g_free(user_path);
        return;
    }
    g_free(user_path);
    ...
    for (int i = 0; sys_dirs && sys_dirs[i]; i++) {
        ...
        if (g_file_test(sys_path, G_FILE_TEST_EXISTS)) {
            _apply_keybinds_from_file(sys_path);
            g_free(sys_path);
            return;
        }
        g_free(sys_path);
...
#  endif
    ...
    /* apply theme overrides from config — works regardless of GTK version */
    {
        ...
        gboolean has_bg = app_config->app_bg_set || app_config->toolbar_bg_set ||
                         ...
                         app_config->sel_bg_set || app_config->side_pane_bg_set;
        gboolean has_fg = app_config->app_fg_set || app_config->toolbar_fg_set ||
                         app_config->pathbar_fg_set || app_config->view_fg_set ||
                         app_config->sel_fg_set || app_config->side_pane_fg_set;
        if (has_bg || has_fg || app_config->app_font)
        {
            GString *css = g_string_new(NULL);
            GdkRGBA *bg, *fg;

/* helper macro: emit the same rule for both normal and :backdrop states
 * so the window looks identical whether focused or not */
#define EMIT_BG(selector, r, g2, b) \
    g_string_append_printf(css, \
        selector " { background-color: #%02x%02x%02x; background-image: none; }\n" \
        selector ":backdrop { background-color: #%02x%02x%02x; background-image: none; }\n", \
        (int)((r)*255),(int)((g2)*255),(int)((b)*255), \
        (int)((r)*255),(int)((g2)*255),(int)((b)*255))
#define EMIT_FG(selector, r, g2, b) \
    g_string_append_printf(css, \
        selector " { color: #%02x%02x%02x; }\n" \
        selector ":backdrop { color: #%02x%02x%02x; }\n", \
        (int)((r)*255),(int)((g2)*255),(int)((b)*255), \
        (int)((r)*255),(int)((g2)*255),(int)((b)*255))

            /* ── window background ── */
            if (app_config->app_bg_set)
                EMIT_BG("window.rdfm-main-win",
                        app_config->app_bg.red,
                        app_config->app_bg.green,
                        app_config->app_bg.blue);
            if (app_config->app_fg_set)
                EMIT_FG("window.rdfm-main-win",
                        app_config->app_fg.red,
                        app_config->app_fg.green,
                        app_config->app_fg.blue);
            ...
            if (bg) {
                EMIT_BG(".rdfm-toolbar", bg->red, bg->green, bg->blue);
                /* kill all GTK border/shadow on toolbar buttons too */
                g_string_append_printf(css,
                    ".rdfm-toolbar { border: none; box-shadow: none; }\n"
                    ".rdfm-toolbar:backdrop { border: none; box-shadow: none; }\n");
            }
            if (fg) {
                EMIT_FG(".rdfm-toolbar", fg->red, fg->green, fg->blue);
                /* new-tab icon and all toolbar image children inherit fg color */
                g_string_append_printf(css,
                    ".rdfm-toolbar image { color: #%02x%02x%02x; }\n"
                    ".rdfm-toolbar image:backdrop { color: #%02x%02x%02x; }\n",
            ...
            fg = app_config->pathbar_fg_set ? &app_config->pathbar_fg
                : app_config->app_fg_set ? &app_config->app_fg : NULL;
            g_string_append(css,
                ".rdfm-pathbar, .rdfm-pathbar * "
                "{ border: none; box-shadow: none; background-image: none; }\n"
                ".rdfm-pathbar:backdrop, .rdfm-pathbar *:backdrop "
                "{ border: none; box-shadow: none; background-image: none; }\n");
            if (bg) {
                EMIT_BG(".rdfm-pathbar", bg->red, bg->green, bg->blue);
                EMIT_BG(".rdfm-pathbar *", bg->red, bg->green, bg->blue);
            }
            if (fg) {
                EMIT_FG(".rdfm-pathbar", fg->red, fg->green, fg->blue);
                EMIT_FG(".rdfm-pathbar *", fg->red, fg->green, fg->blue);
            }
            ...
            fg = app_config->view_fg_set ? &app_config->view_fg
                : app_config->app_fg_set ? &app_config->app_fg : NULL;
            if (bg) {
                EMIT_BG("window.rdfm-main-win .view", bg->red, bg->green, bg->blue);
                EMIT_BG("window.rdfm-main-win .view header button", bg->red, bg->green, bg->blue);
                EMIT_BG("window.rdfm-main-win treeview header button", bg->red, bg->green, bg->blue);
                g_string_append(css,
            ...
            if (fg) {
                EMIT_FG("window.rdfm-main-win .view", fg->red, fg->green, fg->blue);
                EMIT_FG("window.rdfm-main-win .view header button", fg->red, fg->green, fg->blue);
                EMIT_FG("window.rdfm-main-win treeview header button", fg->red, fg->green, fg->blue);
            }
            ...
            if (app_config->sel_bg_set || app_config->sel_fg_set)
            {
                if (app_config->sel_bg_set) {
                    EMIT_BG("window.rdfm-main-win treeview:selected",
                            app_config->sel_bg.red, app_config->sel_bg.green, app_config->sel_bg.blue);
                    EMIT_BG("window.rdfm-main-win treeview row:selected",
                            app_config->sel_bg.red, app_config->sel_bg.green, app_config->sel_bg.blue);
                    EMIT_BG("window.rdfm-main-win .view .cell:selected",
                            app_config->sel_bg.red, app_config->sel_bg.green, app_config->sel_bg.blue);
                }
                if (app_config->sel_fg_set) {
                    EMIT_FG("window.rdfm-main-win treeview:selected",
                            app_config->sel_fg.red, app_config->sel_fg.green, app_config->sel_fg.blue);
                    EMIT_FG("window.rdfm-main-win treeview row:selected",
                            app_config->sel_fg.red, app_config->sel_fg.green, app_config->sel_fg.blue);
                    EMIT_FG("window.rdfm-main-win .view .cell:selected",
                            app_config->sel_fg.red, app_config->sel_fg.green, app_config->sel_fg.blue);
                }
            }

            /* ── side pane (places panel) ──
             * Falls back to app_bg/app_fg.  Also themes the context-menu
             * popup that appears when right-clicking a bookmark, which uses
             * a bare GtkMenu and ignores the window's style context. */
            bg = app_config->side_pane_bg_set ? &app_config->side_pane_bg
                : app_config->app_bg_set ? &app_config->app_bg : NULL;
            fg = app_config->side_pane_fg_set ? &app_config->side_pane_fg
                : app_config->app_fg_set ? &app_config->app_fg : NULL;
            if (bg) {
                EMIT_BG(".rdfm-side-pane", bg->red, bg->green, bg->blue);
                EMIT_BG(".rdfm-side-pane *", bg->red, bg->green, bg->blue);
                /* popup menu from side pane — GTK3 renders menus as
                 * window.popup containing a menu widget; also cover
                 * window.popup > decoration > menu path used by some themes */
                g_string_append_printf(css,
                    "window.popup { background-color: #%02x%02x%02x; }\n"
                    "window.popup > decoration { background-color: #%02x%02x%02x; border: none; box-shadow: none; }\n"
                    "window.popup menu { background-color: #%02x%02x%02x; border: none; }\n"
                    "window.popup menu menuitem { background-color: #%02x%02x%02x; border: none; }\n"
                    "window.popup menu separator { background-color: #%02x%02x%02x; }\n",
            ...
            if (fg) {
                EMIT_FG(".rdfm-side-pane", fg->red, fg->green, fg->blue);
                EMIT_FG(".rdfm-side-pane *", fg->red, fg->green, fg->blue);
                g_string_append_printf(css,
                    "window.popup menu menuitem { color: #%02x%02x%02x; }\n"
                    "window.popup menu menuitem:backdrop { color: #%02x%02x%02x; }\n"
                    "window.popup menu menuitem label { color: #%02x%02x%02x; }\n",
                    (int)(fg->red*255),(int)(fg->green*255),(int)(fg->blue*255),
                    (int)(fg->red*255),(int)(fg->green*255),(int)(fg->blue*255),
                    (int)(fg->red*255),(int)(fg->green*255),(int)(fg->blue*255));
                if (bg)
                    g_string_append_printf(css,
                        ...
                        "window.popup menu menuitem:selected"
...
#undef EMIT_FG
            ...
            /* ── structural borders ──
             * Suppress the light-coloured borders GTK draws around the paned
             * splitter, notebook tabs, and side pane header button so they
             * don't bleed through the dark theme. */
            if (app_config->app_bg_set || app_config->toolbar_bg_set)
            {
                GdkRGBA *b = app_config->app_bg_set
                    ? &app_config->app_bg : &app_config->toolbar_bg;
                g_string_append_printf(css,
                    /* paned splitter handle */
                    "paned > separator { background-color: #%02x%02x%02x; min-width: 1px; min-height: 1px; }\n"
                    /* notebook tab bar background */
                    "notebook > header { background-color: #%02x%02x%02x; border: none; box-shadow: none; }\n"
                    "notebook > header > tabs > tab { background-color: #%02x%02x%02x; border: none; }\n"
                    "notebook > header > tabs > tab:checked { background-color: #%02x%02x%02x; }\n"
                    /* side pane header / mode button */
                    ".rdfm-side-pane button { border: none; box-shadow: none; }\n"
                    ".rdfm-side-pane button:backdrop { border: none; box-shadow: none; }\n",
                    (int)(b->red*255),(int)(b->green*255),(int)(b->blue*255),
                    (int)(b->red*255),(int)(b->green*255),(int)(b->blue*255),
                    (int)(b->red*255),(int)(b->green*255),(int)(b->blue*255),
                    (int)(b->red*255),(int)(b->green*255),(int)(b->blue*255));
            }
